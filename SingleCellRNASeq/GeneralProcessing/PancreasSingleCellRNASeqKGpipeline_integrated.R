...
unique(Idents(T1D_Timepoints))
Idents(T1D_Timepoints)<-T1D_Timepoints$CellSubType
DimPlot(T1D_Timepoints, label = TRUE, label.size = 5, label.box = TRUE, pt.size = 2, repel = TRUE) + 
...
DimPlot(week6_data, label = TRUE, label.size = 6, label.box = TRUE, pt.size = 2, repel = TRUE) + 
  theme(axis.line = element_blank(), 
        axis.ticks = element_blank(), 
        axis.text = element_blank(), 
        axis.title = element_blank())
DimPlot(week12_data, label = TRUE, label.size = 6, label.box = TRUE, pt.size = 2, repel = TRUE) + 
  theme(axis.line = element_blank(), 
        axis.ticks = element_blank(), 
        axis.text = element_blank(), 
        axis.title = element_blank())
...
# Scanpy Conversion T Cell only Compositions----
Tcells_only <- subset(Tcells, subset = TCellSubType != "ILC3" & 
                        TCellSubType != "ILC2" & 
                        TCellSubType != "NK Cell")

saveRDS(Tcells_only, file = "./annotated_Tcells_only_T1D_Timepoints_v1.rds")
library(SeuratData)
library(SeuratDisk)
Tcells_only[["RNA_counts"]] <- CreateAssayObject(counts = GetAssayData(Tcells_only, slot = "counts"))
Tcells_only[["RNA_counts"]]$data
SaveH5Seurat(Tcells_only, filename = "Tcells_only.h5Seurat")
Convert("Tcells_only.h5Seurat", dest = "h5ad",assay = "RNA_counts")
DimPlot(Tcells_only ,group.by = "TCellSubType")
# Extract metadata
metadata_Tcells_only <- Tcells_only@meta.data
metadata_Tcells_only$orig.ident
# Count the number of cells per subtype per sample
cell_counts_Tcells_only <- table(metadata_Tcells_only$sample, metadata_Tcells_only$TCellSubType)

# Convert to dataframe
df_counts_Tcells_only <- as.data.frame(cell_counts_Tcells_only)
colnames(df_counts_Tcells_only) <- c("Sample", "TCellSubType", "Count")

# Calculate total cells per sample
total_cells_Tcells_only <- aggregate(Count ~ Sample, data = df_counts_Tcells_only, sum)
colnames(total_cells_Tcells_only)[2] <- "TotalCells"

# Merge total cell counts with cell type counts
df_counts_Tcells_only <- merge(df_counts_Tcells_only, total_cells_Tcells_only, by = "Sample")

# Compute percentages
df_counts_Tcells_only$Percentage <- (df_counts_Tcells_only$Count / df_counts_Tcells_only$TotalCells) * 100

# View the final dataframe
print(df_counts_Tcells_only)
write.csv(df_counts_Tcells_only,file = "TCell_Counts_T1D_Timepoints.csv")

# Plot T Cell Compositions- CD8,CD4 and OverView ----
getwd()
...
color_list <- colorRampPalette(brewer.pal(9, "Paired"))(6)


# Ensure TCellSubType is a factor
#Tcells$TCellSubType <- factor(Tcells$TCellSubType)  
cell_types<-levels(Idents(Tcells))
...
barplot(
  ...
  width = 1.25, 
  ylab = "Fraction of Lymphoid Population",
  cex.lab = 2,   # Increase font size of axis titles
  cex.names = 2,  # Increase font size of x-axis labels
  cex.axis = 1.5     # Increase font size of axis tick labels
)
...
DimPlot(Tcells,label = T,repel = T,label.size = 8) + 
  scale_color_manual(values = color_list)

# Plot Only T Cell Compositions-Detailed ----

# Subset the data for Week 6 and Week 12 T Cells
Tcells_only<- readRDS("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/SingleCellRNASeq/ProcessedRDSFile/annotated_Tcells_only_T1D_Timepoints_v1.rds")

### CD4----
CD4Tcells_only <- subset(Tcells_only, idents = "CD4 T Cell")

Idents(CD4Tcells_only)<-CD4Tcells_only$TCellMemorySubType

DimPlot(Tcells_only,label=T)

Idents(Tcells_only)<-Tcells_only$TCellSubType



DotPlot(Tcells_only, assay = "SCT", features = c(
  # Pan-T Cell
  "Cd4","Cd8a",
...
VlnPlot(Tcells_only, features = c("Hspa8"),split.by = 'group')
week6_data_CD4TCells <- subset(CD4Tcells_only,subset =time  %in% c("Week6"))
week12_data_CD4TCells <- subset(CD4Tcells_only,subset =time  %in% c("Week12"))


# Create frequency tables for TCells of Week 6 and Week 12 using active.ident and group
freq_week6_CD4TCells <- prop.table(x = table(week6_data_CD4TCells@active.ident,week6_data_CD4TCells@meta.data[, "group"]), margin =2)

freq_week12_CD4TCells <- prop.table(x = table(week12_data_CD4TCells@active.ident,week12_data_CD4TCells@meta.data[, "group"]), margin =2)


library(ggsci)
color_list <- pal_npg("nrc", alpha = 0.8)(11)


# Ensure TCellSubType is a factor
CD4Tcells_only$TCellMemorySubType <- factor(CD4Tcells_only$TCellMemorySubType)  
cell_types<-levels(CD4Tcells_only$TCellMemorySubType)

# Create frequency tables for Week 18 using active.ident and group
# Create a vector of colors for each bar
named_colors<-setNames(color_list,cell_types)
bar_colors <- named_colors[rownames(freq_week6_CD4TCells)]
#rep(color_list, length.out = nrow(freq_week6_TCells))
# Set the resolution to 300 DPI
# Plot for Week 6
par(mar = c(5, 6, 4, 6))  # Increasing the right margin to 6
...
# Plot the bar plot with modified x-axis label rotation
barplot(
  freq_week6_CD4TCells, 
  xlim = c(0, ncol(freq_week6_CD4TCells) + 3),
  col = bar_colors, 
  legend.text = TRUE, 
  args.legend = list(
    x = ncol(freq_week6_CD4TCells) + 3.5, 
    y = max(colSums(freq_week6_CD4TCells)), 
    bty = "n", 
    cex = 1.5
  ), 
  width = 1.25, 
  ylab = "Fraction of CD4 T Cell",
  cex.lab = 2,   # Increase font size of axis titles
  cex.names = 2,  # Increase font size of x-axis labels
  cex.axis = 1.5     # Increase font size of axis tick labels
)

bar_colors <- named_colors[rownames(freq_week12_CD4TCells)]
...
# Plot the bar plot with modified x-axis label rotation
barplot(
  freq_week12_CD4TCells, 
  xlim = c(0, ncol(freq_week12_CD4TCells) + 3),
  col = bar_colors, 
  legend.text = TRUE, 
  args.legend = list(
    x = ncol(freq_week12_CD4TCells) + 3.5, 
    y = max(colSums(freq_week12_CD4TCells)), 
    bty = "n", 
    cex = 1.5
  ), 
  width = 1.25, 
...
DimPlot(CD4Tcells_only,label = T,repel = T,label.size = 6)+ 
  scale_color_manual(values = bar_colors)

all.markers <- FindAllMarkers(CD4Tcells_only , only.pos = TRUE,recorrect_umi = FALSE)
...
write.csv(top20_sct, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/SingleCellRNASeq/Annotations/top20_sct_CD4T_T1D_Timepoints.csv", row.names = FALSE)


#### CD4 Memory Subclustering ----
CD4Memory<-subset(CD4Tcells_only, idents='Tcon memory')

ElbowPlot(CD4Memory, ndims = 50)
CD4Memory <- RunUMAP(CD4Memory, dims = 1:30)
CD4Memory <- FindNeighbors(CD4Memory, dims = 1:30, verbose = F)
CD4Memory <- FindClusters(CD4Memory, res = 0.5)#graph.name = "SCT_nn"


DimPlot(CD4Memory, reduction = "umap", combine = F, label = T)

CD4Memory <- PrepSCTFindMarkers(CD4Memory)
all.markers <- FindAllMarkers(CD4Memory , only.pos = TRUE,recorrect_umi = FALSE)
...
getwd()
# Save the top 20 markers grouped by cluster as a CSV file
write.csv(top20_sct, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/SingleCellRNASeq/Annotations/top20_sct_CD4TMemory_T1D_Timepoints.csv", row.names = FALSE)


DotPlot(CD4Memory, assay = "SCT", features = c(
  # Pan-T Cell
  "Ccr7", "Sell", "Il7r", "Lef1",  
  "Cd40lg","Il6ra","Cxcr3","Cxcr6","Fasl",
  "Slamf6","Tcf7"

)) + 
  RotatedAxis() + scale_colour_gradient2(low = "blue", mid = "grey", high = "red")

#DimPlot(CD8Memory,label=T)
...
genes_of_interest <- c(
  # Pan-T Cell
  "Ccr7", "Sell", "Il7r", "Tcf7",  # T_CM (Central Memory)
  "Cx3cr1", "Gzmb", "Prf1", "Ifng", "Gnly",  # T_EM (Effector Memory)
  "Cd69", "Itgae", "Runx3", "Zfp683", "Cxcr6",  # T_RM (Resident Memory)
  "Klrg1", "B3gat1", "Gzmk","Lag3","Pdcd1"  # T_EMRA (Terminally Differentiated)
) # PTPRC = CD45 (CD45RA is a spliced isoform)

# Generate violin plots
VlnPlot(object = CD8Memory, features = genes_of_interest, group.by = "seurat_clusters", pt.size = 0) 
DoHeatmap(CD8Memory, features = genes_of_interest) + NoLegend()

# Define the T cell subtypes for each cluster
CD8MemorySubtype <- c("CD8 Memory",  # 0
                      "CD8 Central Memory",  # 1
                      "",  # 2
                      "",  # 3
                      "CD8-Effector Memory",  # 4
                      "CD8 Central Memory",  # 5-?
                      "CD8-Tissue Memory Resident", # 6
                      "Cycling / Proliferating T cells",#7
                      "TBD" #8
)        
...
CD8Tcells_only <- subset(Tcells_only, idents = "CD8 T Cell")
Idents(CD8Tcells_only)<-CD8Tcells_only$TCellSubType
DimPlot(CD8Tcells_only)

week6_data_CD8TCells <- subset(CD8Tcells_only,subset =time  %in% c("Week6"))
week12_data_CD8TCells <- subset(CD8Tcells_only,subset =time  %in% c("Week12"))


# Create frequency tables for TCells of Week 6 and Week 12 using active.ident and group
freq_week6_CD8TCells <- prop.table(x = table(week6_data_CD8TCells@active.ident,week6_data_CD8TCells@meta.data[, "group"]), margin =2)

freq_week12_CD8TCells <- prop.table(x = table(week12_data_CD8TCells@active.ident,week12_data_CD8TCells@meta.data[, "group"]), margin =2)


library(ggsci)
color_list <- pal_npg("nrc", alpha = 0.8)(2)


# Ensure TCellSubType is a factor
CD8Tcells_only$TCellSubType <- factor(CD8Tcells_only$TCellSubType)  
cell_types<-levels(CD8Tcells_only$TCellSubType)

# Create frequency tables for Week 18 using active.ident and group
# Create a vector of colors for each bar
named_colors<-setNames(color_list,cell_types)
bar_colors <- named_colors[rownames(freq_week6_CD8TCells)]
#rep(color_list, length.out = nrow(freq_week6_TCells))
# Set the resolution to 300 DPI
# Plot for Week 6
par(mar = c(5, 6, 4, 6))  # Increasing the right margin to 6
...
# Plot the bar plot with modified x-axis label rotation
barplot(
  freq_week6_CD8TCells, 
  xlim = c(0, ncol(freq_week6_CD8TCells) + 3),
  col = bar_colors, 
  legend.text = TRUE, 
  args.legend = list(
    x = ncol(freq_week6_CD8TCells) + 3.5, 
    y = max(colSums(freq_week6_CD8TCells)), 
    bty = "n", 
    cex = 1.5
  ), 
  width = 1.25, 
  ylab = "Fraction of CD8 T Cell",
  cex.lab = 2,   # Increase font size of axis titles
  cex.names = 2,  # Increase font size of x-axis labels
  cex.axis = 1.5     # Increase font size of axis tick labels
)

bar_colors <- named_colors[rownames(freq_week12_CD8TCells)]
...
# Plot the bar plot with modified x-axis label rotation
barplot(
  freq_week12_CD8TCells, 
  xlim = c(0, ncol(freq_week12_CD8TCells) + 3),
  col = bar_colors, 
  legend.text = TRUE, 
  args.legend = list(
    x = ncol(freq_week12_CD8TCells) + 3.5, 
    y = max(colSums(freq_week12_CD8TCells)), 
    bty = "n", 
    cex = 1.5
  ), 
  width = 1.25, 
...
DimPlot(CD8Tcells_only,label = T,repel = T,label.size = 8)+ 
  scale_color_manual(values = bar_colors)

#### CD8 Memory Subclustering ----
CD8Memory<-subset(CD8Tcells_only, idents='CD8 memory')

ElbowPlot(CD8Memory, ndims = 50)
CD8Memory <- RunUMAP(CD8Memory, dims = 1:30)
CD8Memory <- FindNeighbors(CD8Memory, dims = 1:30, verbose = F)
CD8Memory <- FindClusters(CD8Memory, res = 0.5)#graph.name = "SCT_nn"


DimPlot(CD8Memory, reduction = "umap", combine = F, label = T)

CD8Memory <- PrepSCTFindMarkers(CD8Memory)
all.markers <- FindAllMarkers(CD8Memory , only.pos = TRUE,recorrect_umi = TRUE)
...
getwd()
# Save the top 20 markers grouped by cluster as a CSV file
write.csv(top20_sct, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/SingleCellRNASeq/Annotations/top20_sct_CD8TMemory_T1D_Timepoints.csv", row.names = FALSE)


DotPlot(CD8Memory, assay = "SCT", features = c(
  # Pan-T Cell
  "Ccr7", "Sell", "Il7r", "Tcf7",  # T_CM (Central Memory)
  "Cx3cr1", "Gzmb", "Prf1", "Ifng", "Gnly",  # T_EM (Effector Memory)
  "Cd69", "Itgae", "Runx3", "Zfp683", "Cxcr6",  # T_RM (Resident Memory)
  "Klrg1", "B3gat1", "Gzmk"  # T_EMRA (Terminally Differentiated)
)) + 
  RotatedAxis() + scale_colour_gradient2(low = "blue", mid = "grey", high = "red")
...
# Scanpy Conversion T1D_Timepoints for Spectra----
T1D_Timepoints <- readRDS("./annotated_T1D_Timepoints_v4.rds")
library(SeuratData)
library(SeuratDisk)

#Only for Spectra
T1D_Timepoints@assays$SCT$data <- T1D_Timepoints@assays$SCT$counts
# Get the variable features (genes present in scale.data)
variable_features <- rownames(T1D_Timepoints@assays$SCT$scale.data)
subsetted_data <- T1D_Timepoints@assays$SCT$data[variable_features, ]
# Create a new Assay object with the subsetted data
new_assay <- CreateAssayObject(data = subsetted_data)
# Replace the existing SCT assay with the new assay
T1D_Timepoints@assays$RNACounts <- new_assay

# Verify the dimensions
dim(T1D_Timepoints@assays$SCT$scale.data)
dim(T1D_Timepoints@assays$RNACounts$data)

# Ensure that CellSubType is a character
T1D_Timepoints$CellSubType <- as.character(T1D_Timepoints$CellSubType)

# Save the Seurat object
SaveH5Seurat(T1D_Timepoints, filename = "T1D_Timepoints.h5Seurat")

# Convert the Seurat object to an h5ad file for Scanpy
Convert("T1D_Timepoints.h5Seurat", dest = "h5ad", assay = "RNACounts")
