...
keyvals <- ifelse(results_early$padj < 0.1 & results_early$log2FoldChange > 1, "#7E0000",  # Dark red for FDR-significant up
           ifelse(results_early$padj < 0.1 & results_early$log2FoldChange < -1, "#002E66",  # Dark blue for FDR-significant down
           ifelse(results_early$log2FoldChange > 1 & results_early$pvalue < 0.05, "#FF0000",  # Bright red for nominally significant up
...
names(keyvals) <- ifelse(results_early$padj < 0.1 & results_early$log2FoldChange > 1, "FDR < 0.1 & log[2]FC > 1",
                  ifelse(results_early$padj < 0.1 & results_early$log2FoldChange < -1, "FDR < 0.1 & log[2]FC < -1",
...
# Save the plot
ggsave("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/Volcanoplot_Early.png", width = 10, height = 6, dpi = 600)


# Save the data frame as CSV
write.csv(results_early, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/DEGAnalysis_Volcano_Early.csv")

```



## Volcano Analysis- Intermediate
```{r Volcano Analysis- Intermediate}
#### Start Analysis ####

# Step 1: Run DESeq2 for Intermediate and Intermediate groups
# Subset data for Intermediate and Intermediate
...
# Define color mapping using EnhancedVolcano's default shades
# Define color mapping using EnhancedVolcano's default shades
# Define color mapping with stronger contrast for visibility
keyvals <- ifelse(results_Intermediate$padj < 0.1 & results_Intermediate$log2FoldChange > 1, "#7E0000",  # Dark red for FDR-significant up
           ifelse(results_Intermediate$padj < 0.1 & results_Intermediate$log2FoldChange < -1, "#002E66",  # Dark blue for FDR-significant down
           ifelse(results_Intermediate$log2FoldChange > 1 & results_Intermediate$pvalue < 0.05, "#FF0000",  # Bright red for nominally significant up
...
# Assign names for legend
# Assign names for legend
names(keyvals) <- ifelse(results_Intermediate$padj < 0.1 & results_Intermediate$log2FoldChange > 1, "FDR < 0.1 & log[2]FC > 1",
                  ifelse(results_Intermediate$padj < 0.1 & results_Intermediate$log2FoldChange < -1, "FDR < 0.1 & log[2]FC < -1",
...
# Genes to label
#genes_to_label_intermediate <- c("Cd300ld5", "Mlf1", "Mmp3", "Cpt1b", "Elovl6", "Ppp1r1a", "Smtnl2", "Tpm2")
...
EnhancedVolcano(results_Intermediate,
    ...
    ylim = c(0, max(-log10(results_Intermediate$pvalue), na.rm = TRUE) + 1),  
    colCustom = keyvals,  # Apply custom colors
    caption = NULL  # Removes watermark text
)

# Save the plot
ggsave("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/Volcanoplot_Intermediate.png", width = 10, height = 6, dpi = 600)

# Save the data frame as CSV
write.csv(results_Intermediate, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/DEGAnalysis_Volcano_Intermediate.csv")

```



## Volcano Analysis- Late
```{r PCA}
#### Start Analysis ####

# Step 1: Run DESeq2 for Late and Intermediate groups
# Subset data for Late and Intermediate
...
keyvals <- ifelse(results_Late$padj < 0.1 & results_Late$log2FoldChange > 1, "#7E0000",  # Dark red for FDR-significant up
           ifelse(results_Late$padj < 0.1 & results_Late$log2FoldChange < -1, "#002E66",  # Dark blue for FDR-significant down
           ifelse(results_Late$log2FoldChange > 1 & results_Late$pvalue < 0.05, "#FF0000",  # Bright red for nominally significant up
           ifelse(results_Late$log2FoldChange < -1 & results_Late$pvalue < 0.05, "#1E90FF",  # Bright blue for nominally significant down
           "gray"))))  # Gray for non-significant genes

# Assign names for legend
names(keyvals) <- ifelse(results_Late$padj < 0.1 & results_Late$log2FoldChange > 1, "FDR < 0.1 & log[2]FC > 1",
                  ifelse(results_Late$padj < 0.1 & results_Late$log2FoldChange < -1, "FDR < 0.1 & log[2]FC < -1",
...
genes_to_label_late <- c("Ighg2c", "Il31ra", "Tafa2","Cd7", "Il13", "Il17b", "Il17re", "Prf1", "Rorc", "Trbc2", "Tnfrsf17", "Trcg1", "Cd163",    # Classic M2 macrophage scavenger receptor
  ...
  "Cd209d",   # Another DC-SIGN homolog, dendritic cells
  "Smpd3",    # Sphingomyelinase; involved in inflammatory myeloid cell function
  "Sectm1b"  # Secreted and transmembrane protein upregulated in myeloid activation
...
# Save the plot
ggsave("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/Volcanoplot_Late.png", width = 10, height = 6, dpi = 600)

# Save the data frame as CSV
write.csv(results_Late, file = "/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Data/Sequencing/ScaffoldDESEQ/DEGAnalysis_Volcano_Late.csv")
...
# pwl_biocarta <- split(biocarta_gene_sets$gene_symbol, # Genes to split into pathways, by ensembl
#                        biocarta_gene_sets$gs_name) # Pathway names
pwl_msigdbr <- c(pwl_hallmark, pwl_kegg,pwl_reactome) # Compile them all
length(pwl_reactome)
...
### DotPlot-Immune and Metabolic Pathways
```{r DotPlotImmuneSignalling, echo=FALSE}
# Load necessary libraries
library(ggplot2)
library(dplyr)
library(tidyr)
filtered_gsea_metab_df<-as.data.frame(read.csv("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Results/Figure 1/Data/Filtered_GSEA_metab_results.csv"))
...
filtered_gsea_immune_top  <- filtered_gsea_immune_top  %>%
  mutate(timepoint = case_when(
    timepoint == "early" ~ "Early",
    timepoint == "intermediate" ~ "Intermediate",
    timepoint == "late" ~ "Late",
    TRUE ~ timepoint  # Keep other values unchanged
  ))
...
plot <- ggplot(filtered_gsea_immune_top, aes(x = timepoint, y = reorder(ID, NES))) +
  ...
  scale_size(range = c(6, 20), name = bquote(bold("-log10(" * p[adj] * ")"))) +  # Bubble size with formatted p-adj
  theme_minimal() +
  labs(
    x = "Time",
    y = "Pathway",
...
ggsave("/Users/jyotirmoyroy/Desktop/Immunometabolism T1D Paper/Results/Figure 1/Data/Immune_and Metabolic_pathways_Scaffold.png", plot, width = 40, height = 18, dpi = 600)
```
...
filtered_gsea_metab_top30 <- filtered_gsea_metab_top30 %>%
  mutate(timepoint = case_when(
    timepoint == "early" ~ "Early",
    timepoint == "intermediate" ~ "Intermediate",
    timepoint == "late" ~ "Late",
...
plot2<-ggplot(filtered_gsea_metab_top30, aes(x = timepoint, y = reorder(ID, NES))) +
  ...
  theme(
    ...
    axis.text.y = element_text(size = 18),  # Adjust y-axis text size for readability
    axis.title.x = element_text(size = 25),  # Increase x-axis title size
    axis.title.y = element_text(size = 25),  # Increase y-axis title size
    legend.position = "right",
    plot.title = element_text(size = 26, face = "bold")  # Increase plot title size
  )
