...
keyvals <- ifelse(results_early$log2FoldChange > 1 & results_early$pvalue < 0.05, "#B31B21",  # Default red
           ifelse(results_early$log2FoldChange < -1 & results_early$pvalue < 0.05, "#1465AC",  # Default blue
           "gray"))

names(keyvals) <- ifelse(results_early$log2FoldChange > 1 & results_early$pvalue < 0.05, "Upregulated",
                  ifelse(results_early$log2FoldChange < -1 & results_early$pvalue < 0.05, "Downregulated", "NS"))

# Generate the volcano plot without "EnhancedVolcano" watermark
EnhancedVolcano(results_early,
    ...
    widthConnectors = 0.5,
    ylab = '-log10(p-value)', 
    ylim = c(0, max(-log10(results_early$pvalue), na.rm = TRUE) + 1),  
    colCustom = keyvals,  # Apply custom colors
    caption = NULL  # Removes the watermark text
)


# Save the plot as PNG
ggsave("Volcanoplot_Early.png", width = 8, height = 6, dpi = 600)
# Save the merged_results data frame to a CSV file
write.csv(results_early, file = "DEGAnalysis_Volcano_Early.csv", row.names = TRUE)
```
...
# Define color mapping using EnhancedVolcano's default shades
keyvals <- ifelse(results_Intermediate$log2FoldChange > 1 & results_Intermediate$pvalue < 0.05, "#B31B21",  # Default red
           ifelse(results_Intermediate$log2FoldChange < -1 & results_Intermediate$pvalue < 0.05, "#1465AC",  # Default blue
           "gray"))

names(keyvals) <- ifelse(results_Intermediate$log2FoldChange > 1 & results_Intermediate$pvalue < 0.05, "Upregulated",
                  ifelse(results_Intermediate$log2FoldChange < -1 & results_Intermediate$pvalue < 0.05, "Downregulated", "NS"))
...
# Save the plot as PNG
ggsave("Volcanoplot_Intermediate.png", width = 8, height = 6, dpi = 600)
# Save the merged_results data frame to a CSV file
write.csv(results_Intermediate, file = "DEGAnalysis_Volcano_Intermediate.csv", row.names = TRUE)
```
...
# Define color mapping using EnhancedVolcano's default shades
keyvals <- ifelse(results_Late$log2FoldChange > 1 & results_Late$pvalue < 0.05, "#B31B21",  # Default red
           ifelse(results_Late$log2FoldChange < -1 & results_Late$pvalue < 0.05, "#1465AC",  # Default blue
           "gray"))

names(keyvals) <- ifelse(results_Late$log2FoldChange > 1 & results_Late$pvalue < 0.05, "Upregulated",
                  ifelse(results_Late$log2FoldChange < -1 & results_Late$pvalue < 0.05, "Downregulated", "NS"))
...
EnhancedVolcano(results_Late,
    ...
    ylab = '-log10(p-value)',  
     xlim = c(-10,10),
    ylim = c(0, max(-log10(results_Late$pvalue), na.rm = TRUE) + 1),  
    colCustom = keyvals,  # Apply custom colors
    caption = NULL  # Removes the watermark text
)
...
# Save the plot as PNG
ggsave("Volcanoplot_Late.png", width = 8, height = 6, dpi = 600)
# Save the merged_results data frame to a CSV file
write.csv(results_Late, file = "DEGAnalysis_Volcano_Late.csv", row.names = TRUE)
```
